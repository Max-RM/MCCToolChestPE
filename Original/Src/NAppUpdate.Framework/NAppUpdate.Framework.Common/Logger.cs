...
public class Logger
{
	...
	public class LogItem
	{
		...
		public override string ToString()
		{
			if (Exception == null)
			{
				return string.Format("{0,-25}\t{1}\t{2}", Timestamp.ToShortDateString() + " " + Timestamp.ToString("HH:mm:ss.fff"), Severity, Message);
			}
			return string.Format("{0,-25}\t{1}\t{2}{3}{4}", Timestamp.ToShortDateString() + " " + Timestamp.ToString("HH:mm:ss.fff"), Severity, Message, Environment.NewLine, Exception);
		}
	}
	...
	public void Dump(string filePath)
	{
		if (string.IsNullOrEmpty(filePath))
		{
			...
			filePath = Path.Combine(directoryName ?? string.Empty, "NauUpdate.log");
		}
		lock (LogItems)
		{
