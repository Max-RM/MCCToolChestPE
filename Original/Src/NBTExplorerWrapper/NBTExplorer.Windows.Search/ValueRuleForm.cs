...
public class ValueRuleForm : Form
{
	...
	public long TagValueAsLong
	{
		get
		{
			if (long.TryParse(TagValue, out var result))
			{
				return result;
			}
			if (double.TryParse(TagValue, out var result2))
			{
				return (long)result2;
			}
			return 0L;
	...
	public double TagValueAsDouble
	{
		get
		{
			if (double.TryParse(TagValue, out var result))
			{
				return result;
			}
			return 0.0;
	...
	public NumericOperator Operator
	{
		get
		{
			return (NumericOperator)_selectOperator.SelectedItem;
		}
		set
		{
	...
	{
		InitializeComponent();
		foreach (KeyValuePair<NumericOperator, string> @operator in operators)
		{
	...
	private bool TryParseValue()
	{
		...
		if (long.TryParse(TagValue, out var _) || double.TryParse(TagValue, out var _))
		{
			return true;
		}
		return false;
