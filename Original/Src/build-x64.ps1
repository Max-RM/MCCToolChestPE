# Build MCCToolChestPE for Windows x64 (64-bit native)
param(
...
dotnet restore (Join-Path $exeProject "MCCToolChestPE.csproj") -r win7-x64 | Out-Null
& $msbuild (Join-Path $exeProject "MCCToolChestPE.csproj") /p:Configuration=$Configuration /p:PlatformTarget=x64 /p:Prefer32Bit=false /v:minimal
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

$outDir = Join-Path $exeProject "bin\$Configuration\net45"
Write-Host ""
Write-Host "x64 build succeeded."
