...
public class DirectoryDataNode : DataNode
{
	...
	public override string NodePathName
	{
		get
		{
			string text = ((_path.EndsWith("/") || _path.EndsWith("\\")) ? _path : (_path + '/'));
			string text2 = Path.GetDirectoryName(text) ?? text.Substring(0, text.Length - 1);
			int num = Math.Max(text2.LastIndexOf('/'), text2.LastIndexOf('\\'));
			if (num <= 0)
			{
				return text2;
			}
