...
public class FileUtils
{
	...
	public static string FormatLdbKeyForDisplay(string ldbKey)
	{
		if (string.IsNullOrEmpty(ldbKey))
		{
			return ldbKey;
		}
		string text = ldbKey;
		for (int i = 0; i < 4; i++)
		{
			...
			string text2 = DecodeLdbKeyFileName(text);
			if (text2 == text)
			{
				break;
			}
	...
	public static string LdbKeyFromBytes(byte[] keyBytes)
	{
	...
	public static byte[] LdbKeyToBytes(string ldbKey)
	{
		if (string.IsNullOrEmpty(ldbKey))
		{
			return Array.Empty<byte>();
		}
		return Encoding.UTF8.GetBytes(ldbKey);
	}
	...
	public static string EncodeLdbKeyFileName(string ldbKey)
	{
		if (string.IsNullOrEmpty(ldbKey))
		{
			return ldbKey;
		...
		char[] invalid = Path.GetInvalidFileNameChars();
		StringBuilder stringBuilder = new StringBuilder(ldbKey.Length);
		foreach (char c in ldbKey)
		{
			if (Array.IndexOf(invalid, c) >= 0)
			{
				stringBuilder.Append('%').Append(((int)c).ToString("X2", CultureInfo.InvariantCulture));
			}
			else
			{
	...
	public static string DecodeLdbKeyFileName(string encoded)
	{
		...
		{
			if (encoded[i] == '%' && i + 2 < encoded.Length && int.TryParse(encoded.Substring(i + 1, 2), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out int result))
			{
