...
public class MainForm : Form
{
	...
	private PeDataButtonsFrame qN7StructureButtons;

	private PeDataButtonsFrame qN7TickingareaButtons;

	private ToolStripMenuItem qN7PlayerStructuresMenu;

	private ToolStripMenuItem qN7PlayerTickingareasMenu;
	...
	private ToolStripMenuItem closeWorldMenuItem;
	...
	private void IZvSsffrH2l()
	{
		...
		OoLSsUwRB3t(displayUI, false, false, false, false, false, false);
		q0AShUPDFyp.Text = "";
		k2jShLKDVK7.Text = "";
		qK0Shgl81pa.Text = "";
	...
	private void CloseCurrentWorld()
	{
		...
		bool hasChanges = tPRSmnjF0Jx.Controller.CheckModifications() || Y6PSsNQK0GQ() || Working.DataChanged || MPASmUndjLJ.MapsChanged;
		if (hasChanges)
		{
			DialogResult dialogResult = MessageBox.Show(this, Ne4dSgXrbYTX6VcmT1p.mqbSrBrZa1U(90240), Ne4dSgXrbYTX6VcmT1p.mqbSrBrZa1U(90318), MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);
			if (dialogResult == DialogResult.Cancel)
	...
	[MethodImpl(MethodImplOptions.NoInlining)]
	private void OnCloseWorldMenuClick(object sender, EventArgs e)
	{
		while (false)
		{
			_ = ((object[])null)[0];
	...
	private void NmHSsMvVjT5(TreeView P_0)
	{
		...
		if (P_0 != null)
		{
			...
			if (selectedNode != null && selectedNode.Tag != null)
			{
				...
				else if (selectedNode.Tag is string actionTag)
				{
					if (actionTag == PeTreeTags.OpenPlayerMaps)
					{
						nEHSKN5FnnF(this, EventArgs.Empty);
						displayUI = DisplayUI.NONE;
					}
					else if (actionTag == PeTreeTags.OpenStructureManager)
					{
						displayUI = DisplayUI.NONE;
						flag5 = true;
					}
					else if (actionTag == PeTreeTags.OpenTickingareaManager)
					{
						displayUI = DisplayUI.NONE;
						flag6 = true;
					}
				...
				else if (selectedNode.Tag is IndexEntry)
				{
					...
					if (indexEntry.FileName.StartsWith(PeTreeTags.StructureTemplatePrefix, StringComparison.OrdinalIgnoreCase)
					...
					else if (!indexEntry.IsRegion)
					{
						...
						text2 = FileUtils.FormatLdbKeyForDisplay(indexEntry.FileName);
				...
				else if (!flag && !string.IsNullOrWhiteSpace(text2) && tPRSmnjF0Jx.tvNBTEdit.Nodes.Count > 0)
				{
					string extension = Path.GetExtension(text);
					tPRSmnjF0Jx.tvNBTEdit.Nodes[0].Text = text2 + extension;
				}
		...
		OoLSsUwRB3t(displayUI, flag, flag2, flag3, flag4, flag5, flag6);
	}

	[MethodImpl(MethodImplOptions.NoInlining)]
	private void OoLSsUwRB3t(DisplayUI P_0, bool P_1, bool P_2, bool P_3, bool P_4, bool P_5, bool P_6)
	{
		while (false)
		{
			_ = ((object[])null)[0];
		...
		qN7StructureButtons.Visible = P_5;
		qN7TickingareaButtons.Visible = P_6;
	...
	private void fYSSs12pvYk()
	{
		...
		closeWorldMenuItem.Enabled = !string.IsNullOrWhiteSpace(Working.T92StMGt1p4());
	...
	private void CPhSqk0depC()
	{
		...
		if (selectedNode != null && selectedNode.Tag is PERegion)
		{
			...
			if (dialogResult == DialogResult.Yes)
			{
				...
				Working.DataChanged = true;
				fYSSs12pvYk();
				string dimension = Constants.GetDimensionFolderName(pERegion.Dimension);
		...
		if (QA9SqcoVyN5(tagNodeCompound))
		{
			...
			if (tagNodeList != null && tagNodeList2 != null)
			{
				...
				if (chunkDesigner.ShowDialog(this) == DialogResult.OK)
				{
					...
					string dimension = Constants.GetDimensionFolderName(chunkData.Dimension);
	...
	private void h8bSKek8bUh()
	{
		...
		qN7StructureButtons.Top = 0;
		qN7StructureButtons.Left = num - (qN7StructureButtons.Width + 2);
		qN7StructureButtons.Height = num2;
		qN7TickingareaButtons.Top = 0;
		qN7TickingareaButtons.Left = num - (qN7TickingareaButtons.Width + 2);
		qN7TickingareaButtons.Height = num2;
		if (IP0SmgviEFx.Visible || RFPSmkGp6mu.Visible || GkXSmRRWXTn.Visible || BYNSmPqLFo5.Visible || qN7StructureButtons.Visible || qN7TickingareaButtons.Visible)
		{
	...
	[MethodImpl(MethodImplOptions.NoInlining)]
	private void qN7OpenStructureManager(bool allowAdd, string selectedLdbKey = null)
	{
		while (false)
		{
			_ = ((object[])null)[0];
		...
		using PeDataManagerForm peDataManagerForm = new PeDataManagerForm(PeManagedDataKind.Structure, CZCShKpUaJ9, allowAdd, selectedLdbKey);
	...
	[MethodImpl(MethodImplOptions.NoInlining)]
	private void qN7OpenTickingareaManager(bool allowAdd, string selectedLdbKey = null)
	{
		while (false)
		{
			_ = ((object[])null)[0];
		...
		using PeDataManagerForm peDataManagerForm = new PeDataManagerForm(PeManagedDataKind.Tickingarea, CZCShKpUaJ9, allowAdd, selectedLdbKey);
		peDataManagerForm.EntriesChanged += delegate(object sender, PeDataEntriesChangedEventArgs e)
		{
			qN7MarkImportedEntries(e.ImportedKeys);
			fYSSs12pvYk();
	...
	[MethodImpl(MethodImplOptions.NoInlining)]
	private void qN7MarkImportedEntries(IEnumerable<string> ldbKeys)
	{
		while (false)
		{
			_ = ((object[])null)[0];
		...
		foreach (string ldbKey in ldbKeys)
		{
			...
			qN7FindAllTreeNodesByKey(CZCShKpUaJ9.Nodes, ldbKey, list);
			if (list.Count > 0)
			{
				foreach (TreeNode item in list)
				{
					G5fSsR0fB3e(item, FileStateType.MODIFIED);
				}
			}
			else
			{
				qN7MarkImportedEntryDirect(ldbKey);
			}
		}
	}

	[MethodImpl(MethodImplOptions.NoInlining)]
	private void qN7MarkImportedEntryDirect(string ldbKey)
	{
		while (false)
		{
			_ = ((object[])null)[0];
		}
		string stagingPath = Working.T92StMGt1p4();
		if (string.IsNullOrWhiteSpace(stagingPath) || string.IsNullOrWhiteSpace(ldbKey))
		{
			return;
		}
		PeManagedDataKind kind = (ldbKey.StartsWith(PeTreeTags.StructureTemplatePrefix, StringComparison.OrdinalIgnoreCase) ? PeManagedDataKind.Structure : PeManagedDataKind.Tickingarea);
		IndexEntry indexEntry = PeLdbDataHelper.BuildIndexEntry(ldbKey, kind, stagingPath);
		if (!File.Exists(stagingPath + indexEntry.FilePath))
		{
			return;
		}
		if (!KJySh48Y2Cp.ContainsKey(ldbKey))
		{
			ModifiedFile modifiedFile = new ModifiedFile
			{
				Tag = indexEntry,
				FileState = FileStateType.MODIFIED,
				Treenode = qN7FindTreeNodeByKey(CZCShKpUaJ9.Nodes, ldbKey)
			};
			KJySh48Y2Cp.Add(ldbKey, modifiedFile);
			Y9dShRQ2Gc9.Items.Add(modifiedFile);
		}
		else if (KJySh48Y2Cp[ldbKey].FileState != FileStateType.MODIFIED)
		{
			KJySh48Y2Cp[ldbKey].FileState = FileStateType.MODIFIED;
		}
	}

	[MethodImpl(MethodImplOptions.NoInlining)]
	private void qN7FindAllTreeNodesByKey(TreeNodeCollection nodes, string ldbKey, List<TreeNode> matches)
	{
		while (false)
		{
			_ = ((object[])null)[0];
		...
		foreach (TreeNode node in nodes)
		{
			...
			qN7FindAllTreeNodesByKey(node.Nodes, ldbKey, matches);
		}
	}

	[MethodImpl(MethodImplOptions.NoInlining)]
	private TreeNode qN7FindTreeNodeByKey(TreeNodeCollection nodes, string ldbKey)
	{
		while (false)
		{
			_ = ((object[])null)[0];
		...
		foreach (TreeNode node in nodes)
		{
			...
			TreeNode treeNode = qN7FindTreeNodeByKey(node.Nodes, ldbKey);
			if (treeNode != null)
			{
				return treeNode;
			}
	...
	[MethodImpl(MethodImplOptions.NoInlining)]
	private void qN7MarkManagedDataEntriesModified(PeManagedDataKind kind)
	{
		while (false)
		{
			_ = ((object[])null)[0];
		}
		string prefix = (kind == PeManagedDataKind.Structure) ? PeTreeTags.StructureTemplatePrefix : PeTreeTags.TickingareaPrefix;
		foreach (TreeNode item in CZCShKpUaJ9.Nodes)
		{
			qN7MarkManagedDataNode(item, prefix);
		}
	}

	[MethodImpl(MethodImplOptions.NoInlining)]
	private void qN7MarkManagedDataNode(TreeNode node, string prefix)
	{
		while (false)
		{
			_ = ((object[])null)[0];
		...
		{
			G5fSsR0fB3e(node, FileStateType.MODIFIED);
		}
		foreach (TreeNode node2 in node.Nodes)
		{
			qN7MarkManagedDataNode(node2, prefix);
		}
	}

	[MethodImpl(MethodImplOptions.NoInlining)]
	private void qN7StructureEditorClick(object P_0, EventArgs P_1)
	{
		while (false)
		{
			_ = ((object[])null)[0];
		...
		bool allowAdd = false;
		string selectedLdbKey = null;
		TreeNode selectedNode = CZCShKpUaJ9.SelectedNode;
		if (selectedNode != null)
		{
			if (selectedNode.Tag is string tag && tag == PeTreeTags.OpenStructureManager)
			{
				allowAdd = true;
			}
			else if (selectedNode.Tag is IndexEntry indexEntry)
			{
				selectedLdbKey = indexEntry.FileName;
			}
		}
		qN7OpenStructureManager(allowAdd, selectedLdbKey);
	}

	[MethodImpl(MethodImplOptions.NoInlining)]
	private void qN7TickingareaEditorClick(object P_0, EventArgs P_1)
	{
		while (false)
		{
			_ = ((object[])null)[0];
		...
		if (selectedNode != null)
		{
			if (selectedNode.Tag is string tag && tag == PeTreeTags.OpenTickingareaManager)
			{
				allowAdd = true;
			}
			else if (selectedNode.Tag is IndexEntry indexEntry)
		...
		qN7OpenTickingareaManager(allowAdd, selectedLdbKey);
	}

	[MethodImpl(MethodImplOptions.NoInlining)]
	private void qN7PlayerStructuresMenuClick(object P_0, EventArgs P_1)
	{
		qN7OpenStructureManager(allowAdd: true);
	}

	[MethodImpl(MethodImplOptions.NoInlining)]
	private void qN7PlayerTickingareasMenuClick(object P_0, EventArgs P_1)
	{
		qN7OpenTickingareaManager(allowAdd: true);
	}
	...
	private void dsVShSrjUpU()
	{
		...
		closeWorldMenuItem = new ToolStripMenuItem();
		...
		qN7PlayerStructuresMenu = new ToolStripMenuItem();
		qN7PlayerTickingareasMenu = new ToolStripMenuItem();
		...
		qN7StructureButtons = new PeDataButtonsFrame();
		qN7TickingareaButtons = new PeDataButtonsFrame();
		...
		wgXShqQRcuV.DropDownItems.AddRange(new ToolStripItem[8] { AluSmItANiV, aUYShhESBbA, closeWorldMenuItem, JVnSmzjaac5, tvqShJ80nXm, o0ESm3RR5ZG, qhiSmN4iDk2, kOQSmrfC79T });
		...
		closeWorldMenuItem.Image = (Image)componentResourceManager.GetObject(Ne4dSgXrbYTX6VcmT1p.mqbSrBrZa1U(27256));
		closeWorldMenuItem.Name = "closeWorldMenuItem";
		closeWorldMenuItem.Size = new Size(146, 22);
		closeWorldMenuItem.Text = "Close World";
		closeWorldMenuItem.Enabled = false;
		closeWorldMenuItem.Click += OnCloseWorldMenuClick;
		...
		ES0ShOeiX9Y.DropDownItems.AddRange(new ToolStripItem[19]
		{
			iglShC060wL, grMSmTvbeIG, ablSmCDqwQF, pHCSm7aFPhs, v1TSmL4aDXY, UqdShd4QmHG, uFZSmDuOLum, qN7PlayerStructuresMenu, qN7PlayerTickingareasMenu, HNoSmxw1ffF, WrLSmJensx0, BfbSmQhWs3X,
		...
		qN7PlayerStructuresMenu.Name = "qN7PlayerStructuresMenu";
		qN7PlayerStructuresMenu.Size = new Size(148, 22);
		qN7PlayerStructuresMenu.Text = "Player Structures";
		qN7PlayerStructuresMenu.Click += qN7PlayerStructuresMenuClick;
		qN7PlayerTickingareasMenu.Name = "qN7PlayerTickingareasMenu";
		qN7PlayerTickingareasMenu.Size = new Size(148, 22);
		qN7PlayerTickingareasMenu.Text = "Player Tickingareas";
		qN7PlayerTickingareasMenu.Click += qN7PlayerTickingareasMenuClick;
		...
		aKYShnYoo41.Panel2.Controls.Add(qN7TickingareaButtons);
		aKYShnYoo41.Panel2.Controls.Add(qN7StructureButtons);
		...
		qN7StructureButtons.Anchor = AnchorStyles.Top | AnchorStyles.Bottom | AnchorStyles.Right;
		qN7StructureButtons.BackColor = Color.Gainsboro;
		qN7StructureButtons.Location = new Point(760, 0);
		qN7StructureButtons.Name = "qN7StructureButtons";
		qN7StructureButtons.Size = new Size(72, 630);
		qN7StructureButtons.TabIndex = 7;
		qN7StructureButtons.Visible = false;
		qN7StructureButtons.DataEditor += qN7StructureEditorClick;
		qN7TickingareaButtons.Anchor = AnchorStyles.Top | AnchorStyles.Bottom | AnchorStyles.Right;
		qN7TickingareaButtons.BackColor = Color.Gainsboro;
		qN7TickingareaButtons.Location = new Point(760, 0);
		qN7TickingareaButtons.Name = "qN7TickingareaButtons";
		qN7TickingareaButtons.Size = new Size(72, 630);
		qN7TickingareaButtons.TabIndex = 8;
		qN7TickingareaButtons.Visible = false;
		qN7TickingareaButtons.DataEditor += qN7TickingareaEditorClick;
