...
void leveldb_SuspendCompaction(leveldb_t* db) {
  if (db != nullptr && db->rep != nullptr) {
    db->rep->SuspendCompaction();
  }
}

void leveldb_ResumeCompaction(leveldb_t* db) {
  if (db != nullptr && db->rep != nullptr) {
    db->rep->ResumeCompaction();
  }
}
